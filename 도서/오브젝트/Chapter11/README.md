...
* 믹스인은 상속 계층 안에서 확장한 클래스보다 더 하위에 위치하게 되는데 다시 말해서 믹스인은 대상 클래스의 자식 클래스처럼 사용된 용도로 만들어지는 것이다.
* TaxablePolicy와 RateDiscountablePolicy는 BasicRatePolicy에 조합되기 위해 항상 상속 계층의 하위에 믹스인됐고, 따라서 믹스인을 추상 서브클래스라고 부르기도 한다.
* 객체지향 언어에서 슈퍼클래스는 서브클래스를 명시하지 않고도 정의될 수 있지만 서브클래스가 정의될 때는 슈퍼클래스를 명시해야 한다.
* 믹스인은 결론적으로는 슈퍼클래스로부터 상속될 클래스를 명시하는 메커니즘을 표현하기 때문에 하나의 믹스인은 매우 다양한 클래스를 도출하면서 서로 다른 서브클래스를 이용해 인스턴스화될 수 있다.
