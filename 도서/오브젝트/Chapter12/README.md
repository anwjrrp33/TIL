...
## 05. 상속 대 위임
### 위임과 self 참조
* self 참조는 항상 메시지를 수신한 객체를 가리키기 때문에 메서드 탐색 중에는 자식 클래스의 인스턴스와 부모 클래스의 인스턴스가 동일한 self 참조를 공유하는 것으로 봐도 무방하다.
* 상속 계층을 구성하는 객체들 사이에서 self 참조를 공유하기 때문에 개념적으로 각 인스턴스에서 self 참조를 공유하는 self라는 변수를 포함하는 것처럼 표현할 수 있다. 

<img src="./image/그림%2012.21.png">

* GradeLecture의 stats 메서드는 메시지를 직접 처리하지 않고 Lecture의 stats 메서드에게 요청을 전달 한다는 것인데 이처럼 `자신이 수신한 메시지를 다른 객체에게 동일하게 전달해서 처리를 요청 하는 것`을 `위임`이라고 부른다.

#### 포워딩과 위임
* 다른 객체에게 요청을 처리할 때 인자로 self를 전달하지 않을 수도 있지만 메시지를 전송할 필요는 없고 단순히 코드를 재사용하고 싶은 경우라고 할 수 있는데 이처럼 처리를 요청할 때 `self 참조를 전달하지 않는 경우`를 `포워딩`이라고 부른다.

위임은 객체 객체 사이의 동적인 연결 관계를 이용해 상속을 구현하는 방법이며 상속이 매력적인 유는 우리가 직접 구현해야 하는 이런 번잡한 과정을 자동으로 처리 해 준다는 점에서 시작한다.

self 참조의 전달은 결과적으로 자식 클래스의 인스턴스와 부모 클래스의 인스턴스 사이에 동일한 실행 문맥을 공유할 수 있게 해준다. → 코드의 재사용성

상속은 동적으로 메서드를 탐색하기 위해 현재의 실행 문맥을 가지고 있는 self 참조를 전달하고 객체들 사이에서 메시지를 전달하는 과정은 자동으로 이뤄지는데 이를 `자동적인 메시지 위임`이라고 부른다.

### 프로토타입 기반의 객체지향 언어
오직 객체만 존재하는 프로토타입 기반의 객체지향 언어에서 `상속을 구현하는 유일한 방법은 객체 사이의 위임을 이용하는 것`이다.

프로토타입 기반의 객체지향 언어인 자바스크립트를 통해서 객체 사이의 상속이 이뤄지는 것을 확인한다.
* 자바스크립트의 모든 객체들은 다른 객체를 가리키는 용도로 사용되는 prototype이라는 이름의 링크를 가진다.
* prototype은 @parent와 동일한 것으로 봐도 무방하지만 prototype은 self 참조를 직접 전달, 메시지 포워딩을 직접 구현할 필요가 없다.
* 자바스크립트는 prototype 체인으로 연결된 객체 사이에 메시지를 위임함으로써 상속을 구현한다.

<img src="./image/그림%2012.22.png">

객체지향은 객체를 지향하며 클래스는 프로그래밍의 구성 요소일 뿐 중요한 것은 메시지와 협력이다. 클래스 없이도 객체 사이의 협력 관계를 구축하는 것이 가능하며 상속 없이도 다형성을 구현하는 것이 가능하다.

형성 을 위해 클래스 기반의 상속이 널리 사용되지만 프로토타입 언어처럼 위임을 통해 객체 수준에서 상속 을 구현하는 언어들도 존재한다. 클래스 기반의 객체지향 언어를 사용하고 있더라도 클래스라는 제약을 벗어나기 위해 위임 메커니즘을 사용할 수 있다.
